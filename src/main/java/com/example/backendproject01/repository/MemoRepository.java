...
public interface MemoRepository extends JpaRepository<Memo, Long> {
    ...
    // 전체 메모에서 제목 또는 내용으로 검색
    @Query("SELECT m FROM Memo m WHERE m.title LIKE %:keyword% OR m.content LIKE %:keyword% ORDER BY m.createdAt DESC")
    List<Memo> findByKeyword(@Param("keyword") String keyword);

    // 특정 사용자의 메모에서 제목 또는 내용으로 검색
    @Query("SELECT m FROM Memo m WHERE m.author = :author AND (m.title LIKE %:keyword% OR m.content LIKE %:keyword%) ORDER BY m.createdAt DESC")
    List<Memo> findByKeywordAndAuthor(@Param("keyword") String keyword, @Param("author") User author);
